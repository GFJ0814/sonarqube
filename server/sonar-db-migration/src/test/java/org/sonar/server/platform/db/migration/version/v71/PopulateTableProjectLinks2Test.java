...
public class PopulateTableProjectLinks2Test {
  ...
  public void do_not_copy_already_copied_data() throws SQLException {
    ...
    assertThat(db.select("SELECT UUID, NAME, LINK_TYPE, HREF, COMPONENT_UUID, CREATED_AT FROM PROJECT_LINKS2")
      .stream()
      .map(map -> new Tuple(map.get("UUID"), map.get("NAME"), map.get("LINK_TYPE"), map.get("HREF"), map.get("COMPONENT_UUID"), map.get("CREATED_AT")))
  ...
  private void assertProjectLinks2(Tuple... expectedTuples) {
    assertThat(db.select("SELECT NAME, LINK_TYPE, HREF, COMPONENT_UUID, CREATED_AT, UPDATED_AT FROM PROJECT_LINKS2")
      .stream()
      .map(map -> new Tuple(map.get("NAME"), map.get("LINK_TYPE"), map.get("HREF"), map.get("COMPONENT_UUID"), map.get("CREATED_AT"), map.get("UPDATED_AT")))
      .collect(Collectors.toList()))
