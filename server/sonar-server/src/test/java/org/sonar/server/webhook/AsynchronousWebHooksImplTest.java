...
public class AsynchronousWebHooksImplTest {
  ...
  public void send_global_webhooks() {
    ...
    ComponentDto project = componentDbTester.insertPrivateProject().setOrganizationUuid(defaultOrganizationProvider.get().getUuid());
    webhookDbTester.insert(newWebhookDtoForOrganization(defaultOrganizationProvider.get().getUuid()).setName("First").setUrl("http://url1"));
    webhookDbTester.insert(newWebhookDtoForOrganization(defaultOrganizationProvider.get().getUuid()).setName("Second").setUrl("http://url2"));
